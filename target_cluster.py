...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--summary-out", default="cluster_summary.csv", help="Where to write per-cluster summary CSV")
    ...
    # Column overrides (pipe-separated lists)
    p.add_argument("--numeric-cols", default=None, help="Pipe-separated numeric columns to include")
    p.add_argument("--categorical-cols", default=None, help="Pipe-separated categorical columns to include")
    p.add_argument("--text-cols", default=None, help="Pipe-separated text columns to include")
    ...
    p.add_argument("--no-embed-2d", dest="embed_2d", action="store_false", help="Disable 2D projection columns")
    p.add_argument("--proj-method", choices=["umap", "svd"], default="umap", help="Method for 2D projection")
    p.add_argument("--umap-n-neighbors", type=int, default=20, help="UMAP: number of neighbors")
    p.add_argument("--umap-min-dist", type=float, default=0.1, help="UMAP: min_dist")
    p.add_argument("--umap-metric", type=str, default="euclidean", help="UMAP: metric")

    # Nearest neighbors
    p.add_argument("--topk-neighbors", type=int, default=3, help="Top-K neighbors to record per item (in cluster)")
...
class FeatureBlocks:
    ...
    def hstack(self, w_num: float, w_cat: float, w_txt: float) -> sparse.csr_matrix:
        blocks = []
        if self.X_num is not None and self.X_num.shape[1] > 0: blocks.append(self.X_num.multiply(w_num))
        if self.X_cat is not None and self.X_cat.shape[1] > 0: blocks.append(self.X_cat.multiply(w_cat))
        if self.X_txt is not None and self.X_txt.shape[1] > 0: blocks.append(self.X_txt.multiply(w_txt))
        if not blocks: raise ValueError("No features created — check column lists or input data")
...
def _summarize_counts(idx: np.ndarray, names: List[str], top_n: int = 25):
    if idx.size == 0: return []
    counts = np.bincount(idx, minlength=len(names)); order = np.argsort(-counts)
    return [(names[i], int(counts[i])) for i in order if counts[i] > 0][:top_n]

def report_nan_in_dataframe(df, logger: Optional[logging.Logger] = None, label: str = "features_df", show_examples: int = 5):
    ...
    if not nan_cols and not inf_cols:
        logger.info(f"[{label}] No NaN or ±Inf values found in DataFrame with shape {df.shape}."); return
    if nan_cols:
...
def report_nan_in_sparse(X, feature_names: Optional[Sequence[str]] = None, row_index: Optional[Sequence] = None,
                         logger: Optional[logging.Logger] = None, label: str = "X_sparse",
                         show_examples: int = 5, top_n_features: int = 25):
    if sp is None: raise RuntimeError("scipy.sparse not available for report_nan_in_sparse")
    logger = logger or get_logger()
    if not sp.issparse(X):
        logger.warning(f"[{label}] Input is not sparse; skipping sparse diagnostics"); return
    X = X.tocsr(copy=False)
    n_rows, n_cols = X.shape
    names = list(feature_names) if feature_names is not None else [f"f{i}" for i in range(n_cols)]
    if len(names) != n_cols: names = [f"f{i}" for i in range(n_cols)]
    data = X.data
    if data.size == 0: logger.info(f"[{label}] Sparse matrix has 0 nnz; nothing to check."); return
    nan_mask = np.isnan(data); inf_mask = np.isinf(data)
    nnz_per_row = np.diff(X.indptr); row_ids_per_nnz = np.repeat(np.arange(n_rows), nnz_per_row)

    def _log(mask: np.ndarray, kind: str):
        if not mask.any():
            logger.info(f"[{label}] No {kind} values found in sparse data."); return
        bad_col_idx = X.indices[mask]; tops = _summarize_counts(bad_col_idx, names, top_n=top_n_features)
        logger.error(f"[{label}] Found {mask.sum()} {kind} entries affecting {len(np.unique(bad_col_idx))} feature(s). Top offenders:")
        for feat, cnt in tops: logger.error(f"  - {feat}: {cnt} {kind} entries")
        if show_examples:
    ...
    _log(nan_mask, "NaN"); _log(inf_mask, "±Inf")
...
def parse_embedded_json_columns(df: pd.DataFrame, json_candidate_cols: Optional[List[str]] = None) -> pd.DataFrame:
    ...
    if kids_col in df:
        def _price_stats(lst):
            prs, brands, handling, pkg_qty = [], [], [], []
            if isinstance(lst, list):
                for it in lst:
                    try:
                        p = it.get("price", {}).get("current_retail")
                        if p is None: p = it.get("price", {}).get("reg_retail")
                        if p is not None: prs.append(float(p))
                    ...
                    try:
                        ...
                        if isinstance(bullets, list):
                            for b in bullets:
                                ...
                                if m: pkg_qty.append(int(m.group(1))); break
                    except Exception: pass
            return {
...
def clean_enums(df: pd.DataFrame, columns: Optional[Sequence[str]] = None) -> pd.DataFrame:
    ...
    def _canon_origin(val: Any) -> Any:
        ...
        t = re.sub(r"\s+"," ", t).replace(" the usa"," usa")
        if "assem" in t and "usa" in t: return "Assembled in USA (foreign/domestic parts)"
        if "made" in t and "usa" in t and "import" in t: return "Made in USA and Imported"
        if "made" in t and "usa" in t: return "Made in USA"
        if "import" in t: return "Imported"
        return s.strip().title()
    for col in columns:
        if col in df.columns: df[col] = df[col].apply(_canon_origin)
    return df
...
def derive_tariff_risk_flags(df: pd.DataFrame) -> pd.DataFrame:
    ...
    def _risk_from_text(s: Any) -> Optional[int]:
        ...
        t = str(s).lower()
        if "import" in t or "foreign" in t or "assembled" in t: return 1
        if "made in the usa" in t or ("usa" in t and "import" not in t): return 0
        return None
    risk = None
    for c in candidates:
        if c in df.columns:
            risk = df[c].apply(_risk_from_text) if risk is None else risk.fillna(df[c].apply(_risk_from_text))
    df["tariff_risk"] = risk.fillna(0).astype(int)
    return df

def _to_num(s: Optional[pd.Series], index: pd.Index) -> pd.Series:
    if s is None: return pd.Series(np.nan, index=index)
...
def tune_numeric_cols(numeric_cols: Sequence[str], df: pd.DataFrame) -> List[str]:
    ...
    if "pkg_volume_lwh" in df.columns:
        if "pkg_volume_lwh" not in num: num.append("pkg_volume_lwh")
        if "pkg_volume_lwh__missing" in df.columns and "pkg_volume_lwh__missing" not in num: num.append("pkg_volume_lwh__missing")
    ...
    for col in ["data-product-price-current_retail_min__missing","data-product-price-reg_retail_max__missing"]:
        if col in df.columns and col not in num: num.append(col)
    return num
...
class TextEmbedder:
    def __init__(self, mode: str = "tfidf", max_features: int = 12000, min_df=3, svd_dim: int = 256, random_state: int = 42):
        self.mode = mode; self.max_features = max_features; self.min_df = min_df; self.svd_dim = svd_dim
        self.random_state = random_state
        self._tfidf: Optional[TfidfVectorizer] = None
        self._svd: Optional[TruncatedSVD] = None
        self._openai = None
        self._sbert_model = None
        self.out_feature_names_: Optional[List[str]] = None

    def fit_transform(self, texts: List[str]) -> sparse.csr_matrix:
        if self.mode == "tfidf": return self._fit_transform_tfidf(texts)
        elif self.mode == "openai":
            X = self._fit_transform_openai(texts); self.out_feature_names_ = [f"embed_{i}" for i in range(X.shape[1])]; return X
        elif self.mode == "sbert":
            X = self._fit_transform_sbert(texts); self.out_feature_names_ = [f"sbert_{i}" for i in range(X.shape[1])]; return X
        else: raise ValueError(f"Unknown text mode: {self.mode}")

    def transform(self, texts: List[str]) -> sparse.csr_matrix:
        if self.mode == "tfidf":
            X = self._tfidf.transform(texts)
            if self._svd is not None:
                X = self._svd.transform(X); X = sparse.csr_matrix(X)
            return X
        elif self.mode == "openai": return self._embed_openai(texts)
        elif self.mode == "sbert": return self._embed_sbert(texts)
        else: raise ValueError(f"Unknown text mode: {self.mode}")

    def get_feature_names_out(self) -> List[str]:
        if self.out_feature_names_ is None: return []
        return list(self.out_feature_names_)

    def _fit_transform_tfidf(self, texts: List[str]) -> sparse.csr_matrix:
        self._tfidf = TfidfVectorizer(max_features=self.max_features, min_df=self.min_df, ngram_range=(1, 2), strip_accents="unicode")
        ...
        if self.svd_dim and self.svd_dim > 0 and X.shape[1] > self.svd_dim:
            ...
            X = self._svd.fit_transform(X); X = sparse.csr_matrix(X)
    ...
    def _lazy_openai(self):
        if self._openai is not None: return self._openai
        try:
            import openai
            api_key = os.getenv("OPENAI_API_KEY")
            if not api_key: raise RuntimeError("OPENAI_API_KEY env var not set")
            openai.api_key = api_key; self._openai = openai; return self._openai
    ...
    def _embed_openai(self, texts: List[str]) -> sparse.csr_matrix:
        ...
        for i in range(0, len(texts), BATCH):
            ...
            vectors.extend(vecs)
        X = np.vstack(vectors); return sparse.csr_matrix(X)

    def _fit_transform_openai(self, texts: List[str]) -> sparse.csr_matrix: return self._embed_openai(texts)

    # SBERT (optional)
    def _lazy_sbert(self):
        if self._sbert_model is not None: return self._sbert_model
        try:
            from sentence_transformers import SentenceTransformer
            model_name = os.getenv("SBERT_MODEL", "all-MiniLM-L6-v2")
            self._sbert_model = SentenceTransformer(model_name); return self._sbert_model
        except Exception as e:
    ...
    def _fit_transform_sbert(self, texts: List[str]) -> sparse.csr_matrix: return self._embed_sbert(texts)

# -----------------------------
# Feature engineering pipeline
# -----------------------------

def build_feature_blocks(df: pd.DataFrame, numeric_cols: Sequence[str], categorical_cols: Sequence[str], text_cols: Sequence[str],
                         text_mode: str = "tfidf", text_max_features: int = 12000, min_df: float | int = 3,
                         text_svd_dim: int = 256, random_state: int = 42) -> Tuple[FeatureBlocks, List[str]]:
    num_exist = [c for c in numeric_cols if c in df.columns]; X_num = None; num_names: List[str] = []
    if num_exist:
        imputer = SimpleImputer(strategy="median")
        Xn = imputer.fit_transform(df[num_exist].astype(float))
        scaler = StandardScaler(with_mean=False); Xn = scaler.fit_transform(Xn)
        X_num = sparse.csr_matrix(Xn); num_names = [f"num__{c}" for c in num_exist]

    cat_exist = [c for c in categorical_cols if c in df.columns]; X_cat = None; cat_names: List[str] = []
    if cat_exist:
        try:
    ...
    for c in text_cols:
        if c in df.columns: txt_exist.append(c)
        if c + "__parsed" in df.columns: txt_aug_cols.append(c + "__parsed")
    if "category_path" in df.columns: txt_aug_cols.append("category_path")

    X_txt = None; txt_names: List[str] = []
    if txt_exist or txt_aug_cols:
        cols = list(dict.fromkeys(txt_exist + txt_aug_cols))
        texts = assemble_text_corpus(df, cols)
        embedder = TextEmbedder(mode=text_mode, max_features=text_max_features, min_df=min_df, svd_dim=text_svd_dim, random_state=random_state)
...
def inertia_elbow_k(inertias: List[float], k_values: List[int]) -> int:
    if len(inertias) < 3: return k_values[int(np.argmax(inertias))]
    x = np.array(k_values); y = np.array(inertias)
    x_n = (x - x.min()) / (x.max() - x.min() + 1e-9)
    y_n = (y - y.min()) / (y.max() - y.min() + 1e-9)
    p1 = np.array([x_n[0], y_n[0]]); p2 = np.array([x_n[-1], y_n[-1]]); v = p2 - p1; v = v / (np.linalg.norm(v) + 1e-9)
    dists = []
    for xi, yi in zip(x_n, y_n):
        p = np.array([xi, yi]); proj_len = np.dot(p - p1, v); proj = p1 + proj_len * v
        dists.append(np.linalg.norm(p - proj))
    idx = int(np.argmax(dists)); return k_values[idx]

def select_k_via_metrics(X: sparse.csr_matrix, k_min: int, k_max: int, random_state: int,
                         batch: bool = False, batch_size: int = 2048, metrics_out: Optional[str] = None) -> Tuple[int, pd.DataFrame]:
    rows = []; inertias = []; ks = list(range(k_min, k_max + 1))
    for k in ks:
        km = (MiniBatchKMeans(n_clusters=k, random_state=random_state, batch_size=batch_size, n_init=10)
              if batch else KMeans(n_clusters=k, random_state=random_state, n_init=10))
        labels = km.fit_predict(X)
        inertia = km.inertia_; inertias.append(inertia)
        sample_sz = 20000 if X.shape[0] > 20000 else None
        try: sil = silhouette_score(X, labels, metric="euclidean", sample_size=sample_sz, random_state=random_state)
        except Exception: sil = np.nan
        try: ch = calinski_harabasz_score(X.toarray() if sparse.issparse(X) and X.shape[1] <= 5000 else X, labels)
    ...
    def _norm(col):
        v = dfm[col].values.astype(float); m, M = np.nanmin(v), np.nanmax(v)
        if not np.isfinite(m) or not np.isfinite(M) or M - m < 1e-9: return np.zeros_like(v)
        return (v - m) / (M - m + 1e-9)
    dfm["inertia_elbow_k"] = inertia_elbow_k(inertias, ks)
    dfm["sil_norm"] = _norm("silhouette"); dfm["ch_norm"] = _norm("calinski_harabasz")
    dfm["score"] = 0.55*dfm["sil_norm"] + 0.45*dfm["ch_norm"]
    best_idx = int(dfm["score"].idxmax()); best_k = int(dfm.loc[best_idx, "k"])
    ...
    if abs(best_k - elbow_k) >= 2:
        ...
        if s_elbow > 0 and s_elbow >= s_best * 0.98: best_k = elbow_k
    if metrics_out: dfm.to_csv(metrics_out, index=False)
    return best_k, dfm

# -----------------------------
# Clustering helpers (diagnostics + neighbors + labels)
# -----------------------------
...
def add_cluster_diagnostics(df: pd.DataFrame, X: sparse.csr_matrix, labels: np.ndarray, km, id_col: str, topk: int = 3) -> pd.DataFrame:
    df = df.copy()
    df["cluster_id"] = labels.astype(int)

    # distance to own centroid
    D2C = pairwise_distances(X, km.cluster_centers_, metric="euclidean")
    df["cluster_dist"] = D2C[np.arange(X.shape[0]), df["cluster_id"].to_numpy()]
    # outlier flag (95th percentile per cluster)
    df["cluster_outlier"] = df.groupby("cluster_id")["cluster_dist"].transform(lambda s: (s > s.quantile(0.95))).astype(int)

    # init neighbor columns
    topk = int(max(0, topk))
    for k in range(1, topk+1):
        df[f"nn{k}_id"] = None
        df[f"nn{k}_dist"] = np.nan

    # compute neighbors within each cluster (sparse-friendly)
    lbls = df["cluster_id"].to_numpy()
    for cid in np.unique(lbls):
        idx = np.flatnonzero(lbls == cid)
        if len(idx) <= 1: continue
        k_use = min(topk + 1, len(idx))  # +1 for self
        if k_use <= 1: continue
        nbrs = NearestNeighbors(n_neighbors=k_use, metric="euclidean")
        nbrs.fit(X[idx])
        dists, inds = nbrs.kneighbors(X[idx], return_distance=True)
        # first neighbor is self (distance 0)
        ids = df.iloc[idx][id_col].astype(str).to_numpy() if id_col in df.columns else df.index[idx].astype(str)
        for r, i in enumerate(idx):
            # skip column 0 (self)
            for k in range(1, k_use):
                df.at[i, f"nn{k}_id"] = ids[inds[r, k]]
                df.at[i, f"nn{k}_dist"] = float(dists[r, k])
    return df

def add_cluster_labels(df: pd.DataFrame, label_col="cluster_id",
                       name_col="data-product-category-name", fallback_col="category_path") -> pd.DataFrame:
    def _mode(s: pd.Series):
        m = s.mode()
        return m.iat[0] if not m.empty else None
    base = df[name_col] if name_col in df.columns else df.get(fallback_col)
    mode = df.assign(_name=base).groupby(label_col)["_name"].agg(_mode)
    mode.name = "cluster_label"
    return df.merge(mode, left_on=label_col, right_index=True, how="left")

def add_price_band(df: pd.DataFrame, col="data-product-price-reg_retail_max"):
    x = pd.to_numeric(df.get(col), errors="coerce")
    bins = [-np.inf, 10, 25, 50, 100, 250, 500, 1000, np.inf]
    labels = ["<$10","$10–25","$25–50","$50–100","$100–250","$250–500","$500–1000",">$1000"]
    df["price_band"] = pd.cut(x, bins=bins, labels=labels)
    return df

def cluster_summary(df: pd.DataFrame) -> pd.DataFrame:
    g = df.groupby("cluster_id").agg(
        n=("cluster_id","size"),
        label=("cluster_label","first"),
        median_price=("data-product-price-reg_retail_max", lambda s: pd.to_numeric(s, errors="coerce").median()),
        imported_share=("tariff_risk", lambda s: float(np.mean(s==1))),
        outlier_rate=("cluster_outlier", lambda s: float(np.mean(s==1))),
    ).reset_index()
    return g.sort_values(["n","median_price"], ascending=[False, True])

# -----------------------------
# Projection
# -----------------------------

def project_2d(X: sparse.csr_matrix, method: str, random_state: int,
               n_neighbors: int = 20, min_dist: float = 0.1, metric: str = "euclidean") -> Tuple[np.ndarray, str]:
    if method == "umap":
        try:
            import umap  # pip install umap-learn
            reducer = umap.UMAP(n_components=2, n_neighbors=n_neighbors, min_dist=min_dist,
                                metric=metric, random_state=random_state, verbose=False)
            coords = reducer.fit_transform(X); return coords, "umap"
        except Exception as e:
            print(f"[warn] UMAP failed or not installed ({e}); falling back to TruncatedSVD(2).")
    svd = TruncatedSVD(n_components=2, random_state=random_state)
    coords = svd.fit_transform(X); return coords, "svd"
...
KEEP_FOR_DOWNSTREAM = [
    ...
    # Category / brand / type
    "data-product-category-name", "category_path", "cluster_label",
    "data-product-item-primary_brand-name", "data-product-item-product_classification-item_type-name",
    # Price range & current
    "data-product-price-current_retail_min", "data-product-price-reg_retail_max", "price_band",
    ...
    "cluster_dist", "cluster_outlier", "nn1_id", "nn1_dist", "nn2_id", "nn2_dist", "nn3_id", "nn3_dist",
    # 2D projection
    "UMAP1", "UMAP2",
]
...
def write_enriched_csv(df: pd.DataFrame, labels: np.ndarray, out_path: str) -> None:
    ...
    for c in ["data-product-sales_classification_nodes-node_ids"]:
        if c in df_out.columns and c not in ordered_cols: ordered_cols.append(c)
...
def main():
    ...
    for c in ["rating_quality", "rating_value"]:
        if c in df.columns and c not in numeric_cols: numeric_cols.append(c)
    ...
    if cols_for_check: report_nan_in_dataframe(df[cols_for_check], logger=logger, label="features_df")

    print("Building feature blocks ...")
    blocks, feature_names = build_feature_blocks(
        df=df, numeric_cols=numeric_cols, categorical_cols=categorical_cols, text_cols=text_cols,
        text_mode=args.text_mode, text_max_features=args.text_max_features, min_df=args.min_df,
        text_svd_dim=args.text_svd_dim, random_state=args.random_state
    )
    ...
    if args.k is not None and args.k > 1:
        k = args.k; print(f"Using provided k = {k}")
    else:
        k_min, k_max = args.auto_k; print(f"Selecting k in [{k_min}, {k_max}] ...")
        try:
            k, _ = select_k_via_metrics(X=X, k_min=k_min, k_max=k_max, random_state=args.random_state,
                                        batch=args.batch_kmeans, batch_size=args.batch_size, metrics_out=args.metrics_out)
        except ValueError:
            logger.error("Auto-K failed due to invalid values; see diagnostics above.")
            raise
        print(f"Chosen k = {k}")
    ...
    # Determine best ID column for neighbors
    id_candidates = ["data-product-tcin", "_tcin", "data-product-item-primary_barcode", "data-product-item-dpci"]
    id_col = next((c for c in id_candidates if c in df.columns), None) or df.index.name or "index"
    if id_col == "index": df = df.reset_index().rename(columns={"index":"index"})

    # Diagnostics + neighbors + labels + price band
    df = add_cluster_diagnostics(df, X, labels, km, id_col=id_col, topk=args.topk_neighbors)
    ...
    if args.embed_2d:
        coords, used = project_2d(
            X, method=args.proj_method, random_state=args.random_state,
            n_neighbors=args.umap_n_neighbors, min_dist=args.umap_min_dist, metric=args.umap_metric
        )
        df["UMAP1"] = coords[:, 0]; df["UMAP2"] = coords[:, 1]
    ...
    if args.summary_out:
        ...
        cs.to_csv(args.summary_out, index=False)
        print(f"Wrote cluster summary to {args.summary_out}")
